...
from meshprocessing import hdf2fenics, geo2hdf
from subdomains import SubMeshCollection, FacetDomainInfo, SubDomainInfo
...
class Solver:
    ...
    def setup_domain(self, mesh_path, subdomains_path, surfaces_path):
        ...
        boundary_info = [
            FacetDomainInfo("pre", 1, external=True),
            FacetDomainInfo("astrocytes", 2, external=True),
            FacetDomainInfo("post-membrane", 3, external=False),
            FacetDomainInfo("post-terminal", 4, external=False),
            FacetDomainInfo("post-boundary", 5, external=True),
            FacetDomainInfo("empty", 6, external=True)
        ]
        # create meshes
       ...
       # facet_mapping = bndry_mesh.topology().mapping() [self.exterior_mesh.id()].cell_map()
        ...
        # define elements
        P1 = FiniteElement('P', domain.subdomains["interior"].ufl_cell(), 1) # ion concentrations and potentials
        R0 = FiniteElement('R', domain.subdomains['interior'].ufl_cell(), 0) # Lagrange to enforce /int phi_i = 0
        Q1 = FiniteElement('P', domain.boundaries["post-terminal"].ufl_cell(), 1)    # total membrane current I_M

        # create function spaces
        self.Wi = FunctionSpace(domain.subdomains["interior"], MixedElement([P1, R0]))
        self.We = FunctionSpace(domain.subdomains["exterior"], P1)
        self.Wgt = FunctionSpace(domain.boundaries["post-terminal"], Q1)
        self.Wgm = FunctionSpace(domain.boundaries["post-membrane"], Q1)
        self.W = MixedFunctionSpace(self.Wi, self.We, self.Wgt, self.Wgm)
       ...
       # exterior_element_list = [P1]*(self.N_ions + 1)
        ...
        # create function spaces
       # self.Wi = FunctionSpace(domain.subdomains["interior"], MixedElement(interior_element_list))
       # self.We = FunctionSpace(domain.subdomains["exterior"], MixedElement(exterior_element_list))
       # self.Wgt = FunctionSpace(domain.boundaries["post-terminal"], Q1)
       # self.Wgm = FunctionSpace(domain.boundaries["post-membrane"], Q1)
       # self.W = MixedFunctionSpace(self.Wi, self.We, self.Wgt, self.Wgm)
        return
