...
class Solver:
    ...
    def setup_domain(self, mesh_path, subdomains_path, surfaces_path):
        ...
        # get and set mesh
        #mesh = Mesh(mesh_path) 
        with XDMFFile(MPI.comm_world, mesh_path) as xdmf_infile:
            #mesh = dolfin.cpp.mesh.Mesh()
            mesh = Mesh(mesh_path)
            xdmf_infile.read(mesh_path)
            self.mesh = mesh
            
            subdomains = dolfin.MeshValueCollection("size_t", mesh, mesh.topology().dim())
            xdmf_infile.read(mvc_subdomain, subdomains_path)
            self.subdomains = subdomains
            
            surfaces = dolfin.MeshValueCollection("size_t", mesh, mesh.topology().dim()-1)
            xdmf_infile.read(mvc_boundaries, surfaces_path)
        ...
        self.W = FunctionSpaceProduct(self.Wi, self.We, self.Wg)
        return
    ...
    def create_variational_form(self, splitting_scheme=False, dirichlet_bcs=False):
        ...
        # create functions
        (ui, ue, p_IM) = TrialFunction(self.W)  # trial functions
        (vi, ve, q_IM) = TestFunction(self.W)   # test functions
    ...
    def solve_for_time_step(self):
        ...
        # Use cbc.block to rebuild a "standard" PETSc matrix
        AA = block_mat([[A00,A01,A02], [A10,A11,A12], [A20,A21,A22]])
        bb = block_vec(rhs_blocks)

        vec_values = np.hstack([bi.get_local() for bi in bb])
        bb = PETScVector(PETSc.Vec().createWithArray(vec_values))

        AA = self.collapse_to_PETCS(AA)
